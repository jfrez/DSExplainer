...
\end{quote}
\subsection{Dataset Summaries}
\label{sec:dataset_summaries}

On the classic Iris dataset we trained a RandomForest classifier. DSExplainer computed certainty and plausibility for each sample. Feature combinations involving petal length and petal width dominated the top hypotheses, while sepal measurements were less influential. The model achieved 100\% accuracy with an uncertainty mass of only 0.82\%. DSExplainer's summaries align with known morphological traits of the species. The full LLM responses are available in the supplementary material.

For the Breast Cancer Wisconsin dataset, DSExplainer analyzed ten RandomForest predictions. Each tumor was classified correctly with a consistent uncertainty of 2.08\%. Interactions among worst radius, perimeter and area yielded the highest plausibility for malignancy, while certainty often highlighted concave points and texture. These results demonstrate DSExplainer's ability to interpret medical features reliably. Detailed prompts and LLM explanations accompany the supplementary material.
...
To further demonstrate DSExplainer's versatility, we applied it to the Breast Cancer Wisconsin (Diagnostic) dataset. This medical dataset comprises numerical measurements of cell nuclei, with each instance labeled as malignant or benign. Using DSExplainer with a RandomForestClassifier, we generated explanations for 10 random samples, all of which were correctly classified with an overall uncertainty mass of only 2.08\%. An excerpt from the output file \texttt{cancer\_output.txt} is shown below:
